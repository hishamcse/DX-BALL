...
int r = 7;  //radius of the ball
int life = 3;  //total life
...
int ps = 2;        //powerup speed,meaning in which speed powerups will drop out
int shooter_repeat = 0;   // variable for shooter power up,controlling by space
int textin = 0;      //text input
char name[500];    //in which variable text will be input
...
POWER_UPS shooter = { 8, 0, 0 };  //index of the shooter powerup;trigger(0);caught(0)
POWER_UPS bbeam = { 25, 0, 0 };    //index of the big beam powerup;trigger(0);caught(0)
POWER_UPS sbeam = { 65, 0, 0 };    //index of the small beam powerup;trigger(0);caught(0)
POWER_UPS fireball = { 56, 0, 0 }; //index of the fireball powerup;trigger(0);caught(0)
POWER_UPS_CO shc, bbc, sbc, fic; //powerups co ordinates

char brickl1[3][30] = { "brick 1.bmp", "brick 9.bmp", "brick 7.bmp" };
char brickl2[3][30] = { "brick 3.bmp", "brick 4.bmp", "brick 6.bmp" };
char brickl3[3][30] = { "brick 2.bmp", "brick 5.bmp", "brick 8.bmp" };
...
void powerup(){
	...
	if (!present[shooter.brick] && !shooter.trigger){
		...
		shc.l = 10;    //powerup length will be 10 then which was 0 before for not showing that
...
void ballChange(){
	...
	if (timer){
		if (ball_x >= bpx&&ball_x <= bpx + sw&&ball_y >= 0 && ball_y <= bpy + 7 + sl&&dy<0){//collision of beam & ball
			...
			for (int i = 0; i<j; ++i){
				...
				if ((ball_x + r >= bricks[i][0] && ball_x + r <= bricks[i][0] + bw&&ball_y + r >= bricks[i][1] && ball_y + r <= bricks[i][1] + bl) ||
					...
					if (!fireball.caught) dy = -dy; //fireball powerup never allows ball to reflect
					--brickcount;
					point += 10;
					sprintf(s, "%d", point);
					break;
		...
		ball_x += dx, ball_y += dy;   //movement of ball
	}
}
...
void createl1(){
	...
	skip_row1 = 2 + rand() % 2; //gap at th no row
	skip_row2 = 5 + rand() % 3;
	int vd = 3 + rand() % 4; //gap between some cols happen
	gapx = 2;
	gapy = 2;
...
void createl2(){
	...
	offsety = 40;
	layer = 9+rand()%2;
	int skip_row = 3 + layer % 4 + layer % 3;
	if (layer % 2 == 0) ++layer;
	int n1 = 1;
	int i23 = bl + offsety;
	int rev1 = 0;
	while (n1 <= layer && n1>0){
		...
		if (n1 % 2){
			bricks[j][0] = resx / 2 - bw / 2; //if n1 is odd,creation of the center
			bricks[j++][1] = resy - i23;
			++brickcount;
			int j1 = j - 1; //index of the center brick
			for (int i = 1; i <= (n1 - 1) / 2; ++i){
				bricks[j][0] = bricks[j1][0] - i*bw;
				bricks[j++][1] = resy - i23;
				bricks[j][0] = bricks[j1][0] + i*bw; //reflection of other brick
				bricks[j++][1] = resy - i23;
				brickcount += 2;
			}
		}
			...
			for (int i = 1; i <= n1 / 2; ++i){
				bricks[j][0] = resx / 2 - i*bw; //if n1 is even,creating brick at the center
				bricks[j++][1] = resy - i23;
				bricks[j][0] = resx / 2 + (i - 1)*bw;
				bricks[j++][1] = resy - i23;
				brickcount += 2;
			}
		}
...
void iDraw()
{
	...
	if (State == 2){
		...
		iShowBMP(530, 450, "150px-DX-Ball_-_W32_-_Album_Art.bmp");
	...
	if (State == 3){
		...
		iText(377, 367, name, GLUT_BITMAP_TIMES_ROMAN_24);
		return;
	}
	...
	if (fireball.caught) iSetColor(255, 255, 0);
...
int main()
{
	...
	iPauseTimer(0);
	iSetTimer(10, powerup);
	iPauseTimer(1);
	iSetTimer(10, Shooter);
	iPauseTimer(2);
	iSetTimer(40, animationchange);
	iPauseTimer(3);
	iSetTimer(3, startanimationchange);
	iSetTimer(1000, menuchange);
	setpaddle();
	bpx = 260;
	bpy = 15;
	ball_x = bpx + 50;
